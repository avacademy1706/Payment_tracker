...
`npm run vercel-build` (what Vercel itself runs) does this too, but Vercel's zero-config `/api` function detection appears to read from the committed source rather than from build output, so don't rely on that alone — commit the regenerated file.
...
2. **Import the GitHub repo into Vercel** (New Project → pick `avacademy1706/Payment_tracker`, **Root Directory left blank** — the repo root, not `client/`). Vercel will detect `vercel.json` and use it — no manual build/output configuration needed.
